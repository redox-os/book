...
Each library (dependency) needs to have it's own recipe and added on the `recipe.toml` of the recipe before compilation (dependencies = []), as recipes will fetch sources most of the time, Redox don't have the `-dev/-devel` (runtime/sources) separation seen in most Unix/Linux packages.
