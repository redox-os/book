...
| `CONFIG_NAME` | Determines part of the Redox image name, and is normally used to construct the `FILESYSTEM_CONFIG` name (`desktop` by default). |
| `FILESYSTEM_CONFIG` | A file that describes the packages and files to include in the filesystem. See the [Filesystem Configuration](#filesystem-configuration) section below. The default is `config/$ARCH/$CONFIG_NAME.toml`, but this can be changed if the desired config file is in a different location. |
| `QEMU_MEM` | Sets the QEMU memory quantity, e.g., `QEMU_MEM=2048`. |
...
The Redox build system supports cross-compilation to other CPU architectures. The CPU architecture that Redox is built for&mdash;specified by the `ARCH` environment variable&mdash;usually determines the filesystem configuation file that will be used by the build system.
...
The following describes the process for creating a custom filesystem configuration file (`desktop-test.toml`):
