...
Then, in a loop:
 ...
 4. If it's a scheme request, read a request packet from the scheme file descriptor and call the "handler". The request packet will indicate if it's an `open`, `read`, `write`, etc. on the scheme:
